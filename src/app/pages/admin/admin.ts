...
openAddUserModal() {
console.log('Ação: Abrir modal/form para Adicionar novo Usuário.');
alert('Funcionalidade Adicionar Usuário - Simulação');
}

editUser(user: User) {
console.log('Ação: Abrir modal/form para Editar Usuário:', user.nome, user.id);
alert(`Funcionalidade Editar Usuário - Simulação para: ${user.nome}`);
}

deleteUser(userId: number) {
if (confirm('Tem certeza que deseja DELETAR este usuário? Esta ação é irreversível (ID: ' + userId + ').')) {
console.log('Ação: Deletar Usuário com ID:', userId);

// Lógica para remover o usuário da lista local usando Sinais
this.usersList.update(users => users.filter(user => user.id !== userId));

alert(`Usuário ID ${userId} deletado (simulado)!`);
}
}

// ==================== FUNÇÕES DE AÇÃO (VEÍCULOS) ====================

openAddVehicleModal() {
console.log('Ação: Abrir modal/form para Adicionar novo Veículo.');
alert('Funcionalidade Adicionar Veículo - Simulação');
}

editVehicle(vehicle: Vehicle) {
console.log('Ação: Abrir modal/form para Editar Veículo:', vehicle.placa, vehicle.id);
alert(`Funcionalidade Editar Veículo - Simulação para: ${vehicle.placa}`);
}

deleteVehicle(vehicleId: number) {
if (confirm('Tem certeza que deseja DELETAR este veículo? Esta ação é irreversível (ID: ' + vehicleId + ').')) {
console.log('Ação: Deletar Veículo com ID:', vehicleId);

// Lógica para remover o veículo da lista local usando Sinais
this.vehiclesList.update(vehicles => vehicles.filter(vehicle => vehicle.id !== vehicleId));

alert(`Veículo ID ${vehicleId} deletado (simulado)!`);
}
}
}